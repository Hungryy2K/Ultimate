...
-- Rest des bisherigen Shop-GUI-Codes ...

local ShopGUI = {}

--[[
ShopGUI.show("Itemname", 100, function()
    outputChatBox("Gekauft!")
end)
]]

function ShopGUI.show(itemName, price, onBuy)
    UniversalGUI.create{
        title = "Shop",
        logo = "images/shop_icon.png",
        labels = {
            {text = "Artikel: " .. itemName, color = {255,255,255}},
            {text = "Preis: " .. price .. "$", color = {0,200,0}},
        },
        buttons = {
            {text = "Kaufen", callback = function() if onBuy then onBuy() end end},
        },
        onClose = function() outputChatBox("Shop geschlossen!") end
    }
end
