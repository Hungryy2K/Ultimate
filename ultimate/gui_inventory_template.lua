...
-- Rest des bisherigen Inventory-GUI-Codes ...
...
InventoryGUI.show({
    ...
    outputChatBox("Benutzt: "..item.name)
end, function(item)
    outputChatBox("Weggeworfen: "..item.name)
end)
]]

function InventoryGUI.show(items, onUse, onDrop)
    local btns = {}
    for _, item in ipairs(items) do
        table.insert(btns, {text = item.name .. " benutzen", callback = function() if onUse then onUse(item) end end})
        table.insert(btns, {text = item.name .. " wegwerfen", callback = function() if onDrop then onDrop(item) end end})
    end
    UniversalGUI.create{
        title = "Inventar",
        logo = "images/inventory_icon.png",
        labels = {
            {text = "Deine Items:", color = {255,255,255}},
        },
        buttons = btns,
        onClose = function() outputChatBox("Inventar geschlossen!") end
    }
end
