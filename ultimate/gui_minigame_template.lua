...
-- Rest des bisherigen Minigame-GUI-Codes ...

local MinigameGUI = {}

--[[
MinigameGUI.show("Tetris", function()
    outputChatBox("Tetris gestartet!")
end, function()
    outputChatBox("Tetris abgebrochen!")
end)
]]

function MinigameGUI.show(minigameName, onStart, onCancel)
    UniversalGUI.create{
        title = minigameName,
        logo = "images/minigame_icon.png",
        labels = {
            {text = "Bereit für " .. minigameName .. "?", color = {255,255,255}},
        },
        buttons = {
            {text = "Start", callback = function() if onStart then onStart() end end},
            {text = "Abbrechen", callback = function() if onCancel then onCancel() end end},
        },
        onClose = onCancel
    }
end
