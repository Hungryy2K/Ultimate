...
-- Rest des bisherigen Settings-GUI-Codes ...
...
SettingsGUI.show({
    ...
    outputChatBox("Gespeichert: "..toJSON(newSettings))
end)
]]

function SettingsGUI.show(settings, onSave)
    local sx, sy = guiGetScreenSize()
    local wnd = UniversalGUI.create{
        title = "Einstellungen",
        logo = "images/settings_icon.png",
        labels = {
            {text = "Passe deine Einstellungen an:", color = {255,255,255}},
        },
        buttons = {},
        onClose = function() outputChatBox("Einstellungen geschlossen!") end
    }
    local edits = {}
    local y = sy*0.38
    for key, value in pairs(settings) do
        local lbl = createStyledLabel(sx*0.42, y, sx*0.08, sy*0.04, key..":", wnd)
        local edit = guiCreateEdit(sx*0.51, y, sx*0.12, sy*0.04, tostring(value), false, wnd)
        guiSetFont(edit, "default-bold-small")
        edits[key] = edit
        y = y + sy*0.05
    end
    local saveBtn = createStyledButton(0.35, 0.8, 0.3, 0.13, "Speichern", wnd)
    addEventHandler("onClientGUIClick", saveBtn, function()
        local newSettings = {}
        for key, edit in pairs(edits) do
            newSettings[key] = guiGetText(edit)
        end
        if onSave then onSave(newSettings) end
        if isElement(wnd) then destroyElement(wnd) end
        showCursor(false)
        setElementClicked(false)
    end, false)
